        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="btn-group shadow-sm overflow-hidden rounded-0">
                ...
                <a href="{{ route('invoices.print', $invoice) }}" target="_blank"
                    class="btn btn-primary btn-sm px-3 rounded-0">
                        ...
                        <div class="card-body p-5">
                            <div class="d-flex justify-content-between mb-5">
                                ...
                                <div class="text-end">
                                    ...
                                    <p class="text-muted small mb-0">{{ __('Date') }}:
                                        {{ $invoice->date->format($appSettings['date_format'] ?? 'd/m/Y') }}</p>
                                    <p class="text-muted small">{{ __('Due Date') }}:
                                        {{ $invoice->expired_date->format($appSettings['date_format'] ?? 'd/m/Y') }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="row mb-5">
                                ...
                                <div class="col-6 text-end">
                                    ...
                                    @endphp
                                    <span
                                        class="badge {{ $statusClass }} rounded-pill px-3">{{ strtoupper($invoice->status) }}</span>
                                </div>
                            </div>
                            ...
                            <div class="row justify-content-end mt-5">
                                <div class="col-md-5">
                                    <div class="d-flex justify-content-between mb-2">
                                        ...
                                        <span class="fw-bold">{{ $invoice->currency }}
                                            {{ number_format($invoice->sub_total, 2) }}</span>
                                    </div>
                                    <div class="d-flex justify-content-between mb-2">
                                        <span class="text-muted">{{ __('Tax') }}
                                            ({{ $invoice->tax_rate }}%):</span>
                                        <span class="fw-bold">{{ $invoice->currency }}
                                            {{ number_format($invoice->tax_total, 2) }}</span>
                                    </div>
                                    <hr class="my-3">
                                    <div class="d-flex justify-content-between">
                                        <h5 class="fw-bold">{{ __('Total') }}:</h5>
                                        <h4 class="fw-bold text-primary">{{ $invoice->currency }}
                                            {{ number_format($invoice->total, 2) }}</h4>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            ...
                            @forelse($invoice->payments as $payment)
                                <div class="d-flex align-items-center mb-3">
                                    ...
                                    <div>
                                        <div class="fw-bold small">{{ $invoice->currency }}
                                            {{ number_format($payment->amount, 2) }}</div>
                                        <div class="text-muted x-small">
                                            {{ $payment->date->format($appSettings['date_format'] ?? 'd/m/Y') }}</div>
                                    </div>
                                </div>
                            @empty
