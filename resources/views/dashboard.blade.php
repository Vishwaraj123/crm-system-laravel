<x-app-layout>
    ...
    @php
        ...
        $sym = $appSettings['currency_symbol'] ?? '';

        $invoiceLabels  = ['draft'=>'Draft','pending'=>'Pending','overdue'=>'Overdue','paid'=>'Paid','unpaid'=>'Unpaid','partially'=>'Partially'];
        $proposalLabels = ['draft'=>'Draft','pending'=>'Pending','sent'=>'Sent','accepted'=>'Accepted','declined'=>'Declined','cancelled'=>'Expired'];

        $statusColors = [
            'draft'=>'bg-secondary','pending'=>'bg-warning','overdue'=>'bg-danger',
            'paid'=>'bg-success','unpaid'=>'bg-dark','partially'=>'bg-info',
            'sent'=>'bg-info','accepted'=>'bg-success','declined'=>'bg-danger','cancelled'=>'bg-dark',
        ];
    @endphp
                ...
                <div class="col-md-5">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body">
                            ...
                            @php $count = $stats['invoice_counts'][$key] ?? 0; $pct = round(($count / $invoiceTotal) * 100); @endphp
                            <div class="mb-3">
                ...
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body">
                            ...
                            @php $count = $stats['proposal_counts'][$key] ?? 0; $pct = round(($count / $proposalTotal) * 100); @endphp
                            <div class="mb-3">
                                <div class="d-flex justify-content-between small mb-1">
                                    <span class="text-muted">{{ $label }}</span>
                                    <span class="fw-bold">{{ $pct }} %</span>
            ...
            <div class="row g-4">
                ...
                <div class="col-md-6">
                    <div class="card border-0 shadow-sm">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="px-3 small fw-bold">
                                                <a href="{{ route('invoices.show', $inv) }}" class="text-decoration-none">{{ $inv->number }}</a>
                                            </td>
                                            <td class="small">{{ $inv->client->name }}</td>
                                            <td class="small">{{ $sym }}{{ number_format($inv->total, 2) }}</td>
                                            <td class="small">
                                                <span class="badge {{ $statusColors[$inv->status] ?? 'bg-secondary' }} text-capitalize">{{ $inv->status }}</span>
                                            </td>
                                        </tr>
                                        @empty
                                        <tr>
                        ...
                        <div class="card-footer bg-white border-top-0 text-center py-2">
                            <a href="{{ route('invoices.index') }}" class="btn btn-sm btn-link text-decoration-none small">{{ __('View All') }}</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="card border-0 shadow-sm">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="px-3 small fw-bold">
                                                <a href="{{ route('proposals.show', $prop) }}" class="text-decoration-none">{{ $prop->number }}</a>
                                            </td>
                                            <td class="small">{{ $prop->client->name }}</td>
                                            <td class="small">{{ $sym }}{{ number_format($prop->total, 2) }}</td>
                                            <td class="small">
                                                <span class="badge {{ $statusColors[$prop->status] ?? 'bg-secondary' }} text-capitalize">{{ $prop->status }}</span>
                                            </td>
                                        </tr>
                                        @empty
                                        <tr>
                        ...
                        <div class="card-footer bg-white border-top-0 text-center py-2">
                            <a href="{{ route('proposals.index') }}" class="btn btn-sm btn-link text-decoration-none small">{{ __('View All') }}</a>
                        </div>
                    </div>
                </div>
