<x-app-layout>
                        ...
                        <div class="card-body p-4">
                            <form method="POST" action="{{ route('payments.store') }}">
                                ...
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="number" class="form-label">{{ __('Payment Number') }} <span class="text-danger">*</span></label>
                                            <input type="text" class="form-control @error('number') is-invalid @enderror" id="number" name="number" value="{{ old('number', 'PAY-' . strtoupper(Str::random(6))) }}" required>
                                            @error('number')
                                    ...
                                    <div class="col-md-12">
                                        <div class="mb-3">
                                            <label for="invoice_id" class="form-label">{{ __('Invoice') }} <span class="text-danger">*</span></label>
                                            <select class="form-select @error('invoice_id') is-invalid @enderror" id="invoice_id" name="invoice_id" required>
                                                ...
                                                @foreach ($invoices as $invoice)
                                                    <option value="{{ $invoice->id }}" data-client-id="{{ $invoice->client_id }}" {{ old('invoice_id') == $invoice->id ? 'selected' : '' }}>
                                                        {{ $invoice->number }} - {{ __('Remaining') }}: {{ number_format($invoice->total - $invoice->paid, 2) }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            @error('invoice_id')
                                ...
                                <div class="d-flex justify-content-end gap-2 mt-4">
                                    <a href="{{ route('payments.index') }}" class="btn btn-secondary px-4">{{ __('Cancel') }}</a>
                                    <button type="submit" class="btn btn-primary px-4">{{ __('Record Payment') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const invoiceOptions = Array.from(invoiceSelect.options).slice(1);
            ...
            clientSelect.addEventListener('change', function() {
                ...
                invoiceOptions.forEach(option => {
                    if (!clientId || option.getAttribute('data-client-id') === clientId) {
