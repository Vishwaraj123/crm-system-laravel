...
$(document).ready(function () {
  ...
  // ✅ Bind filters ONCE and send all .filterable inputs
  ...
  $(document).on("click", "#reset", function () {
    filterCount = 0; // reset counter to avoid ID conflict if needed
    $("#filter-builder").empty();
    $(".filterable").val("").trigger("change");
    $(".filter-field").val("").trigger("change");
    $("#pipeline_select").val("all").trigger("change");
    $("stage_select").val("all").trigger("change");
    $("#checkAll").prop("checked", false).trigger("change");
    $("#dataTableBuilder").DataTable().ajax.reload();
  });
});
...
$(document).on("click", "#campaignBTN", function () {
  const parsedFilters = parseFiltersFromDom(); // Your own function

  $.ajax({
    url: filteredContactsUrl,
    method: "GET",
    data: { filters: parsedFilters },
    traditional: true,
    success: function (response) {
      if (response.contact_ids && response.contact_ids.length > 0) {
        const contactIds = response.contact_ids.join(",");
        const url = campaignRouteUrl + "?contact_ids=" + encodeURIComponent(contactIds);

        // Redirect
        window.open(url, '_blank');
      } else {
        alert("No contacts found for the selected filters.");
      }
    },
    error: function (xhr) {
...
function parseFiltersFromDom() {
  const filters = {};

  $(".filterable").each(function () {
    const fullKey = $(this).attr("name") || $(this).attr("id");
    const value = $(this).val();

    // Match pattern filters[1][contact_list_id]
    const match = fullKey.match(/^filters\[(\d+)]\[(\w+)]$/);
    if (match) {
      const index = match[1];
      const field = match[2];
  ...
  console.log("Parsed Filters Array:", result);
